...
variable "proxmox_api_url" {
    type = string
    default = "https://192.168.88.217:8006/api2/json"
}
...
source "proxmox-iso" "ubuntu-server-jammy" {
    ...
    vm_id = "100"  # Уникальный идентификатор для виртуальной машины
    ...
    iso_checksum = "84aeaf7823c8c61baa0ae862d0a06b03409394800000b3235854a6b38eb4856f"  # Контрольная сумма ISO для проверки целостности
    ...
    disks {
        ...
        format = "qcow2"  # Формат диска (qcow2 поддерживает снимки (snapshots))
        storage_pool = "local-lvm"  # Хранилище для диска VM (в данном случае LVM на локальном хранилище)
        storage_pool_type = "lvm"  # Тип хранилища (LVM для быстрой работы с виртуальными дисками)
    ...
    boot_wait = "5s"  # Задержка перед загрузкой системы
