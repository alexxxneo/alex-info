terraform {
    ...
    required_providers {
        proxmox = {
            ...
            version = ">= 2.9.14" # Определяет провайдера Proxmox с указанием источника и версии. Будет скачана версия 2.9.14 или выше провайдера 'telmate/proxmox'.
        }
    }
}
...
provider "proxmox" {
  pm_api_url = "https://192.168.88.217:8006/api2/json"
  pm_user = "root@pam"
...
resource "proxmox_vm_qemu" "proxmox_vm" {
  count       = 2  # Создает 2 виртуальные машины (значение 'count' задает количество экземпляров ресурса).
  name        = "tf-vm-${count.index}"  # Имя каждой виртуальной машины будет 'tf-vm-0', 'tf-vm-1' и т.д. в зависимости от индекса.
  ...
  cores       = 1  # Количество процессорных ядер для каждой виртуальной машины.
  ...
  memory      = 1024  # Объем оперативной памяти в мегабайтах для каждой виртуальной машины (1 ГБ).
  ...
  ipconfig0 = "ip=192.168.88.1${count.index + 1}/24,gw=192.168.88.1"
