...
resource "vkcs_compute_instance" "sng_1" {
  # count = 2 можно указать количество создаваемых ВМ
  name                    = "sng_1"
  flavor_id               = data.vkcs_compute_flavor.compute.id # берется из блока data "vkcs_compute_flavor" где посылается запрос в вк чтобы узнать id типа виртуальной машины по значению переменной сompute_flavor указанной в блоке переменных
  key_pair                = data.vkcs_compute_keypair.sng.id # пара ключей. берется уже существующая из облака
...
##### Указываем участников балансировки и их вес  ####
